  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isSignIn) {
        ...
        const { error, role, needsEmailVerification } = await signUp(email, password, selectedRole, fullName);
        if (error) {
...

        {
          view === 'details' && (
            <div className="space-y-3">
              ...
              >
                {loading ? t('login.please_wait') : needsSetup ? t('login.finish_setup') : isSignIn ? t('login.sign_in') : t('login.create_account_btn')}
              </Button>
              ...
              {!needsSetup && (
                <>
                  ...
                  >
                    ...
                    {isSignIn ? t('login.google_signin') : t('login.google_signup')}
                  </Button>
                  <Button
                    onClick={handleAppleSignIn}
                    disabled={loading}
